...
import NumberPacker, { pack, unpack, calculateNeededBits, calculateMaxVal, NUMBER } from './number-packer.ts';
...
describe('NumberPacker', () => {
    ...
    describe('Untested Public API', () => {
        it('should get the correct range for a schema entry', () => {
            packer.schemaGroups([[1, [
                { bit: 4, signed: false },
                { bit: 8, signed: true },
                { bit: 32, signed: false }, // Becomes a BigInt because bit > 31
            ]]]);
            expect(packer.getRange(0)).toEqual({ min: 0, max: 15 });
            expect(packer.getRange(1)).toEqual({ min: -128, max: 127 });
            expect(packer.getRange(2)).toEqual({ min: 0n, max: (1n << 32n) - 1n });
            expect(packer.getRange(3)).toBeUndefined(); // Out of bounds
        });

        it('should call onOverflow when a value is out of range', () => {
            const onOverflowSpy = vi.fn();
            packer.schemaGroups([[1, [{ bit: 4, signed: false }]]]);
            packer.onOverflow = onOverflowSpy;

            packer.pack([16]); // 4-bit unsigned max is 15

            expect(onOverflowSpy).toHaveBeenCalledTimes(1);
            expect(onOverflowSpy).toHaveBeenCalledWith({
                value: 16,
                index: 0,
                min: 0,
                max: 15,
    ...
    describe('Offset Functionality', () => {
        ...
        it('should pack data within a given buffer length (bufLen)', () => {
            packer.schemaGroups([[1, [255]]]); // 1 byte
            const buffer = new Uint8Array([0, 0, 0, 0]);
            // Pack 1 byte at offset 1, but limit the writable area to 2 bytes total (offset 1 + length 1)
            packer.pack([99], buffer, 1, 2);
            expect(buffer).toEqual(new Uint8Array([0, 99, 0, 0]));
        });
